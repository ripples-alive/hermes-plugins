...
class OpenAICompatibleImageProvider(ImageGenProvider):
    ...
    def get_setup_schema(self):
        return {
            ...
            "badge": "custom",
            "tag": "OpenAI-compatible image endpoint; preset-driven; requests b64_json.",
            "env_vars": [],
        }
    ...
    def _base_url(self, cfg: Dict[str, Any]) -> str:
        ...
        custom_name = _first_str(ax.get("custom_provider"), ax.get("provider_name"))
        matching_custom = None
        if isinstance(custom, list) and custom_name:
            for item in custom:
                if isinstance(item, dict) and str(item.get("name", "")).lower() == custom_name.lower():
                    matching_custom = item
                    break
        return _first_str(
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if response.status_code >= 400:
            ...
            return error_response(
                error=f"OpenAI-compatible image API returned HTTP {response.status_code}: {body}",
        ...
        if not b64:
            return error_response(
                error="OpenAI-compatible image response did not include b64_json; URL responses are intentionally not used here",
        ...
        try:
            ...
            ext = _detect_ext(raw)
            saved = save_b64_image(b64, prefix=f"openai_compatible_image_{_safe_name(preset_name)}_{_safe_name(model)}", extension=ext)
        except Exception as exc:
            return error_response(
                error=f"Could not save returned base64 image: {exc}",
                error_type="io_error",
                provider=self.name,
                model=model,
                prompt=prompt,
